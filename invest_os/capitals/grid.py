...
from invest_os.models.schemas import CapitalType, CapitalScore, CapitalGridResult, Action, CAPITAL_WEIGHTS


class CapitalGrid:
    def __init__(self):
        self.weights = CAPITAL_WEIGHTS
        self.registry = DimensionRegistry.defaults()
        self.limits = {
            CapitalType.VIVO: ("shannon_h", 1.2, "Monocultura — Shannon H' < 1.2"),
            CapitalType.SOCIAL: ("gini", 0.5, "Plutocracia — Gini > 0.5"),
        }

    def add_dimension(self, name: str, weight: float = 0.125,
                      metric: str = "", threshold: Optional[float] = None,
                      bloqueio_msg: str = "") -> CapitalDimension:
        return self.registry.add(name, weight, metric, threshold,
    ...
    def evaluate(
        ...
        for dim in self.registry.dimensions.values():
            if dim.threshold is not None and dim.name not in ("vivo", "social"):
                val = scores.get(CapitalType[dim.name.upper()], 0.5) if dim.name.upper() in CapitalType.__members__ else 0.5
                if dim.metric and dim.name.upper() not in CapitalType.__members__:
                    continue
